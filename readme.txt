ModManagerInstaller is a Linux Game Library and Mod Manager Installer.

Current state:
Active development of UX/UI of main window.
Expecting to have settings menu done next.
Tested on Linux Mint.
Other distro's test results will be added later.

LinMan is made in Godot 4.3-stable.
It offers various install and managment solutions for Games and Mod Managers.
